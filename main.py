#%%
from gibbs import get_mean, get_median, plot_cov_ellipse, gmm_generate, hmm_generate, mvn_logpdf
import numpy as np
...
class Mixture(Module):
    ...
    def _check_data(self,logl):
        self.T, self.N, components = logl.shape
        if components != self.components:
            raise ValueError("input must have same dimensonality as z (N x C)")
        if self.z != logl.shape[:2]:
            self._parameters['z'] = np.random.randint(0,self.components,(self.T,self.N))
...
#%%
# Have data always be 3D (T X N X M)
# For non temporal, T=1.
# Mask with it to say which N are at T, mask = TXN
N = 200
np.random.seed(123)
t = np.arange(N)
y = np.exp(1j*2*np.pi*4*t/100)
y[N//2:] = np.exp(1j*2*np.pi*2*t[N//2:]/100-1j*np.pi)
...
#%%
np.random.seed(123)
model = SLDS(output_dim=1,state_dim=2,states=2,parameter_sampling=True,hyper_sample=True,full_covariance=False,expected_duration=50)
sampler = Gibbs()
...
chain = sampler.get_chain(flatten=False,burn_rate=.5)

fig,ax = plt.subplots(len(chain),figsize=(5,len(chain)*1.5))
ax = np.atleast_1d(ax)
for j,s in enumerate(chain):
    if (s == 'hmm.z') or (s=='lds.x'):
        _chain = np.swapaxes(chain[s],0,1)
        _chain = _chain.reshape(_chain.shape[0],-1)
    else:
        _chain = chain[s].reshape(chain[s].shape[0],-1)
    ax[j].plot(_chain,'k',alpha=.1,linewidth=1)
    ax[j].set_title(s)
...
y_samp = np.zeros((N,clen,model.output_dim,))
for s in range(clen):
    for t in range(N):
